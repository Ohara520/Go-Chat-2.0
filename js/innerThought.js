...
function _deglueThought(txt) {
  ...
  const _isW = (sub) => {
    ...
    if (_IT_WORDS.has(sub)) return true;
    ...
    for (const suf of _suf) {
      if (sub.length > suf.length + 1 && sub.endsWith(suf)) {
        ...
        if (_IT_WORDS.has(base)) return true;
        if ((suf==='ing'||suf==='ed'||suf==='er') && _IT_WORDS.has(base+'e')) return true;
        if ((suf==='ing'||suf==='ed'||suf==='er') && base.length>=3 && base[base.length-1]===base[base.length-2] && _IT_WORDS.has(base.slice(0,-1))) return true;
        if (suf==='ies' && _IT_WORDS.has(base+'y')) return true;
      }
    }
    return false;
  };
