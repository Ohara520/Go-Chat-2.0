  ...
  const _seg = (run) => {
    ...
    for (let i = n - 1; i >= 0; i--) {
      for (let k = Math.min(18, n - i); k >= 1; k--) {
        ...
        const isW = (k === 1) ? (sub === 'i' || sub === 'a' || sub === 'o') : _IT_WORDS.has(sub);
    ...
    while (i < n) {
      let L = 0;
      for (let k = Math.min(18, n - i); k >= 2; k--) { if (_IT_WORDS.has(low.slice(i, i + k))) { L = k; break; } }
      if (!L && (low[i] === 'i' || low[i] === 'a')) { if (i + 1 >= n || _IT_WORDS.has(low.slice(i+1, i+3)) || _IT_WORDS.has(low.slice(i+1, i+4))) L = 1; }
