...
// ===== 打工系统 =====
const WORK_JOBS = [
  { id: 'study',   icon: '📚', name: '学习',  minutes: 25, pay: 20 },
  { id: 'sport',   icon: '💪', name: '运动',  minutes: 30, pay: 25 },
  { id: 'create',  icon: '🎨', name: '创作',  minutes: 45, pay: 35 },
  { id: 'slack',   icon: '😴', name: '摸鱼',  minutes: 10, pay: 5  },
];
...
function getTodayWorkCount() {
  const key = 'workCount_' + new Date().toISOString().slice(0,10);
  return parseInt(localStorage.getItem(key) || '0');
}
function addTodayWork() {
  const key = 'workCount_' + new Date().toISOString().slice(0,10);
  const count = getTodayWorkCount() + 1;
  localStorage.setItem(key, count);
  // 检查本周打工次数
...
function renderWorkScreen() {
  const todayCount = getTodayWorkCount();
  const remaining = Math.max(0, 4 - todayCount);
  const countEl = document.getElementById('workTodayCount');
  const remainEl = document.getElementById('workRemaining');
  if (countEl) countEl.textContent = '£' + getWorkTodayEarned();
  if (remainEl) remainEl.textContent = `（今日还可完成 ${remaining} 个）`;

  // 渲染工作卡片
  const grid = document.getElementById('workJobGrid');
  if (!grid) return;
  grid.innerHTML = WORK_JOBS.map(job => `
    <div class="work-job-card ${workSelected === job.id ? 'selected' : ''} ${todayCount >= 4 ? 'disabled' : ''}"
         onclick="selectJob('${job.id}')">
      <div class="work-job-icon">${job.icon}</div>
      <div class="work-job-name">${job.name}</div>
      <div class="work-job-desc">${job.minutes}分钟 · £${job.pay}</div>
    </div>
  `).join('');

  const startBtn = document.getElementById('workStartBtn');
  if (startBtn) {
    startBtn.disabled = todayCount >= 4 || !workSelected;
    startBtn.textContent = todayCount >= 4 ? '今日已完成' : '开始打工';
  }
}

function getWorkTodayEarned() {
  const key = 'workEarned_' + new Date().toISOString().slice(0,10);
  return parseInt(localStorage.getItem(key) || '0');
}

function selectJob(jobId) {
  if (getTodayWorkCount() >= 4) return;
  workSelected = jobId;
  renderWorkScreen();
}

function startWork() {
  if (!workSelected || getTodayWorkCount() >= 4) return;
  const job = WORK_JOBS.find(j => j.id === workSelected);
  if (!job) return;

  const btn = document.getElementById('workStartBtn');
  const timerEl = document.getElementById('workTimerDisplay');
  let seconds = job.minutes * 60;
...
function finishWork(job) {
  const count = addTodayWork();
  setBalance(getBalance() + job.pay);
  addTransaction({ icon: job.icon, name: `打工 · ${job.name}`, amount: job.pay });
  const earnedKey = 'workEarned_' + new Date().toISOString().slice(0,10);
  localStorage.setItem(earnedKey, getWorkTodayEarned() + job.pay);
  renderWallet();
  workSelected = null;
  const timerEl = document.getElementById('workTimerDisplay');
  if (timerEl) timerEl.style.display = 'none';
  renderWorkScreen();
  showToast(`✅ 打工完成！+£${job.pay}`);
}

// ===== 单词系统 =====
const VOCAB_POOL = [
  { en: 'Exhausted',   zh: '精疲力竭' },
  { en: 'Classified',  zh: '机密的' },
  { en: 'Persistent',  zh: '坚持不懈的' },
  { en: 'Reckless',    zh: '鲁莽的' },
  ...
  { en: 'Concealed',   zh: '隐藏的' },
  { en: 'Lethal',      zh: '致命的' },
  { en: 'Calculated',  zh: '经过深思熟虑的' },
  { en: 'Brilliant',   zh: '出色的（曼彻斯特口语）' },
  { en: 'Proper',      zh: '非常/地道（曼彻斯特口语）' },
  { en: 'Manic',       zh: '疯狂的（北英格兰口语）' },
  { en: 'Sitrep',      zh: '情况汇报（军事）' },
  { en: 'Oscar Mike',  zh: '出发了（军事）' },
  { en: 'Go Dark',     zh: '断联/保持无线电静默' },
  { en: 'Stand Down',  zh: '解除警戒/放松' },
  { en: 'Klicks',      zh: '公里（军事俚语）' },
  { en: 'Extracted',   zh: '已撤离' },
  { en: 'Compromised', zh: '暴露了/被发现了' },
];

function getTodayVocab() {
  const key = 'vocabToday_' + new Date().toISOString().slice(0,10);
  const saved = localStorage.getItem(key);
  if (saved) return JSON.parse(saved);
  // 随机抽5个
  const shuffled = [...VOCAB_POOL].sort(() => Math.random() - 0.5).slice(0, 5);
  localStorage.setItem(key, JSON.stringify(shuffled));
  return shuffled;
}

function getVocabStreak() {
  return parseInt(localStorage.getItem('vocabStreak') || '0');
}
function getTodayVocabProgress() {
  const key = 'vocabProgress_' + new Date().toISOString().slice(0,10);
  return JSON.parse(localStorage.getItem(key) || '{"studied":false,"tested":false,"score":0}');
}
function saveTodayVocabProgress(data) {
  const key = 'vocabProgress_' + new Date().toISOString().slice(0,10);
...
function renderVocabScreen() {
  ...
  const progress = getTodayVocabProgress();
  const streak = getVocabStreak();

  const streakEl = document.getElementById('vocabStreak');
  const progressEl = document.getElementById('vocabProgressCount');
  if (streakEl) streakEl.textContent = streak;
  if (progressEl) progressEl.textContent = (progress.studied ? (progress.tested ? '5' : '5') : '0') + '/5';

  const wordList = document.getElementById('vocabWordList');
  if (wordList) {
    wordList.innerHTML = vocab.map(w => `
      <div class="vocab-word-item">
        <span class="vocab-en">${w.en}</span>
        <span class="vocab-zh">${w.zh}</span>
      </div>
    `).join('');
  }

  const studyBtn = document.getElementById('vocabStudyBtn');
  const testBtn = document.getElementById('vocabTestBtn');
  if (studyBtn) {
    studyBtn.disabled = progress.studied;
    studyBtn.textContent = progress.studied ? '✅ 已学习' : '📖 开始学习';
  }
  if (testBtn) {
    testBtn.disabled = !progress.studied || progress.tested;
    testBtn.textContent = progress.tested ? `✅ 已测验 ${progress.score}/5` : '✏️ 开始测验';
  }
}

function completeVocabStudy() {
  const progress = getTodayVocabProgress();
  if (progress.studied) return;
  progress.studied = true;
  saveTodayVocabProgress(progress);
  changeAffection(0.5);
  // 更新连续天数
  const lastStudy = localStorage.getItem('vocabLastStudy');
  const today = new Date().toISOString().slice(0,10);
  const yesterday = new Date(Date.now() - 86400000).toISOString().slice(0,10);
  if (lastStudy === yesterday) {
    localStorage.setItem('vocabStreak', getVocabStreak() + 1);
  } else if (lastStudy !== today) {
    localStorage.setItem('vocabStreak', 1);
  }
  localStorage.setItem('vocabLastStudy', today);
  renderVocabScreen();
  showToast('📖 学习完成！好感度+0.5');
}

function startVocabTest() {
  const vocab = getTodayVocab();
  const progress = getTodayVocabProgress();
  if (!progress.studied || progress.tested) return;

  let current = 0;
  let score = 0;
  const testArea = document.getElementById('vocabTestArea');
  if (!testArea) return;
  ...
  function showQuestion() {
    if (current >= vocab.length) {
      ...
      saveTodayVocabProgress(progress);
      if (score >= 4) changeAffection(0.5); // 4/5或5/5才加好感
      renderVocabScreen();
      testArea.innerHTML = `<div class="vocab-test-result">测验完成！${score}/5 ${score >= 4 ? '🎉 好感度+0.5' : '💪 继续加油'}</div>`;
      showToast(`✏️ 测验完成 ${score}/5！`);
      return;
    }
    const word = vocab[current];
    // 随机生成3个错误选项
    const wrong = VOCAB_POOL.filter(w => w.en !== word.en)
      .sort(() => Math.random() - 0.5).slice(0, 3).map(w => w.zh);
    const options = [word.zh, ...wrong].sort(() => Math.random() - 0.5);
    testArea.innerHTML = `
      <div class="vocab-test-question">
        <div class="vocab-test-word">${word.en}</div>
        <div class="vocab-test-options">
          ${options.map(opt => `
            <button class="vocab-test-opt" onclick="answerVocab('${opt}','${word.zh}',this)">
              ${opt}
            </button>
          `).join('')}
        </div>
        <div class="vocab-test-progress">${current+1} / ${vocab.length}</div>
      </div>
    `;
  }

  window.answerVocab = function(selected, correct, btn) {
    const btns = testArea.querySelectorAll('.vocab-test-opt');
